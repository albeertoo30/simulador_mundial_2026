...
DESEMPATE EN FASE DE GRUPOS (versión simplificada):
  1º Puntos
  2º Diferencia de goles
  3º Goles a favor
  4º Resultado del enfrentamiento directo 
  5º Sorteo aleatorio 
"""
import pandas as pd
import numpy as np
...
def normalizar_nombre(equipo, dict_elos):
    """Si 'equipo' no está tal cual en dict_elos, prueba la equivalencia conocida."""
...
def cargar_rankings(ruta_csv):
    """Carga el CSV de Elo y lo convierte en un diccionario para búsquedas rápidas."""
...
def simular_partido(equipo_a, equipo_b, dict_elos, media_goles, es_eliminatoria=False):
    """
    Simula un partido usando el Rating Elo y la distribución de Poisson.
    Si es_eliminatoria=True, incluye prórroga y penaltis en caso de empate.
    """
...
# 4. FASE DE GRUPOS
# ============================================================
def jugar_grupo(nombre_grupo, equipos, dict_elos):
    """
    Juega todos los partidos (todos contra todos, ida única) de un grupo
    de 4 equipos simulándolos desde cero y devuelve la tabla ordenada.
    """
    tabla = {
    ...
    for i in range(len(equipos)):
        for j in range(i + 1, len(equipos)):
            ...
            res = simular_partido(eq_a, eq_b, dict_elos, es_eliminatoria=False)
...
def jugar_fase_de_grupos(dict_elos):
    """Juega los 12 grupos y devuelve un DataFrame con todas las tablas."""
    tablas = []
    for nombre_grupo, equipos in GRUPOS_MUNDIAL_2026.items():
        equipos_normalizados = [normalizar_nombre(e, dict_elos) for e in equipos]
        tabla = jugar_grupo(nombre_grupo, equipos_normalizados, dict_elos)
        tablas.append(tabla)
...
def calcular_clasificados(tabla_grupos):
    """Extrae los 24 equipos primeros o segundos y los 8 mejores terceros."""
    primeros_segundos = {}
    terceros = []
...
def construir_dieciseisavos(primeros_segundos, mejores_terceros):
    """Construye los 16 cruces de dieciseisavos de final."""
...
def jugar_ronda_eliminatoria(cruces, dict_elos, nombre_ronda):
    """Juega una ronda eliminatoria completa."""
    ganadores = []
    detalle = []
    for eq_a, eq_b in cruces:
        res = simular_partido(eq_a, eq_b, dict_elos, es_eliminatoria=True)
...
def jugar_fase_eliminatoria(dieciseisavos, dict_elos):
    """Encadena todas las rondas eliminatorias hasta la final."""
    historial = []

    ganadores_16, det = jugar_ronda_eliminatoria(dieciseisavos, dict_elos, "Dieciseisavos")
    historial += det

    octavos = [(ganadores_16[i], ganadores_16[i + 1]) for i in range(0, len(ganadores_16), 2)]
    ganadores_8, det = jugar_ronda_eliminatoria(octavos, dict_elos, "Octavos")
    historial += det

    cuartos = [(ganadores_8[i], ganadores_8[i + 1]) for i in range(0, len(ganadores_8), 2)]
    ganadores_4, det = jugar_ronda_eliminatoria(cuartos, dict_elos, "Cuartos")
    historial += det

    semis = [(ganadores_4[i], ganadores_4[i + 1]) for i in range(0, len(ganadores_4), 2)]
    ganadores_2, det = jugar_ronda_eliminatoria(semis, dict_elos, "Semifinal")
    historial += det

    final = [(ganadores_2[0], ganadores_2[1])]
    campeon, det = jugar_ronda_eliminatoria(final, dict_elos, "FINAL")
    historial += det
...
# ============================================================
def simular_mundial_una_vez(dict_elos, verbose=False):
    tabla_grupos = jugar_fase_de_grupos(dict_elos)
    ...
    campeon, historial = jugar_fase_eliminatoria(dieciseisavos, dict_elos)
...
# ============================================================
def simular_mundial_completo(ruta_rankings_elo, n_simulaciones=10000):
    ...
    for i in range(n_simulaciones):
        campeon, _ = simular_mundial_una_vez(dict_elos, verbose=False)
...
if __name__ == "__main__":
    df_resultado = simular_mundial_completo(
        ...
        n_simulaciones=10000,   # sube esto a 10000+ para el resultado definitivo
    )
